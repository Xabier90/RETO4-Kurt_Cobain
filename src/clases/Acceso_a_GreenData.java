 ...
 * Conecta con una base de datos MySQL para validar el acceso según los usuarios
 * y contraseñas que tenemos en la base de datos.
 * 
...
public class Acceso_a_GreenData {
	...
	//
	//Definimos aqui el enlace a la base de datos, y el usuario y contraseña que
	//hace que conecte con la base de datos. Todo esto hace posible que podamos
	//trabajar con la base de datos.
	
	// private static final String URL =
	// "jdbc:mysql://nas.latorreg.es:3306/futuretech_db";
	// private static final String USER = "xabi";
	// private static final String PASS = "1234";
	/**
	 * Definimos la variable que nos servirá para acotar los intentos de entrar al
	...
	/**
	 * Metodo para cambiar de idioma del Castellano al Euskara. Usamos una variable
	 * booleana para, cambiar de idioma ESP == true en este caso los mensajes se
	 * verán en castellano y si es false en Euskara.
	 */
	...
	/**
	 * El método para confirmar el acceso a GreenData Consulta la base de datos para
	 * verificar si los datos metidos por el usuario (Usuario y Contraseña) son
	 * válidas.
	 * 
	 * @param nombre El nombre de usuario introducido.
	 * @param codigo La contraseña introducida.
	 * @return true si el usuario existe y la contraseña coincide, false en caso
	 *         contrario.
	 */
	private static boolean validarEnMySQL(String nombre, String codigo) {
		// Ajusta 'usuarios', 'username' y 'clave', 'passwd' a tus nombres reales en la
		// tabla
		String consulta = "SELECT * FROM usuarios WHERE username = ? AND passwd = ?";

		/**
		 * La estructura try (...) asegura que la conexión (conn) y el objeto ps se
		 * cierren automáticamente al terminar, evitando fugas de memoria.
		 */
		// try (Connection conn = DriverManager.getConnection(URL, USER, PASS);

		ConexionJoseba conexion = new ConexionJoseba();
		Connection conn = conexion.ActivarConexion();
		...
		try (PreparedStatement ps = conn.prepareStatement(consulta)) {
			...
			conexion.CerrarConexion(conn); // cierras al terminar
			return resultado;
			/**
			 * El bloque catch captura cualquier fallo de conexión o sintaxis SQL, imprime
			 * el error en consola y retorna false para que el programa no se detenga.
			 */
	...
	public static void iniciar() {
		...
		// Este punto lo he tenido que buscar porque no me salía con los apuntes que
		// teniamos
		// Y además me daba error en los label "JLabel LabelUsuario = new
		// JLabel("Usuario:");"
		// y lo he dejado como me ha dicho la IA "LabelUsuario = new
		// JLabel("Usuario:");" quitando JLabel
		...
		// Evento listener para el selector de idiomas: cambia los textos base al hacer
		// clic
		...
		// JPasswordField para ocultar los caracteres de la contraseña mientras se
		// escriben
		...
		/**
		 * Añade un ActionListener al botón. Este bloque de código se ejecuta cada vez
		 * que el usuario hace clic en el botón. e es el evento que se genera al pulsar.
		 * Esto es muy parecido a JavaScript
		 */
		botonAcceso.addActionListener(e -> {
			/**
			 * Obtiene el texto que el usuario escribió en la cuadricula (fieldtext) de
			 * usuario.
			 */
			String personaAutorizada = usuarioField.getText();
			/**
			 * Se obtiene la contraseña escrita y se convierte a String para poder
			 * compararla en la base de datos.
			 */
			String contraseñaAcceso = new String(contraseñaField.getPassword());

			/**
			 * Hace una consulta en la base de datos y comprueba si el usuario y contraseña
			 * existe en la base de datos, si no existe, manda un false, y si existe un
			 * true. Por eso creamos una variable booleana para guardar el resultado de la
			 * consulta
			 */
			boolean accesoValido = validarEnMySQL(personaAutorizada, contraseñaAcceso);

			/**
			 * Si el usuario y contraseña son correctos, cambia el texto informativo tanto
			 * en contenido como en color "verde".
			 */
			if (accesoValido) {
			...
			/**
			 * Si el usuario o contraseña son incorrectas, la variable intentos se
			 * incrementa o suma 1 y te sale un mensaje de color rojo, diciendo los intentos
			 * que te quedan para poder acceder al programa de monitoreo (para meter el
			 ...
			 */
				...
				/**
				 * La condición de que introduzcas mal o el nombre de usuario o la contraseña
				 * hará que se desactiva el botón. El usuario ya no puede volver a intentar
				 * acceder y le aparecerá un mensaje dejando claro que se ha bloqueado el
				 * acceso.
				 */
